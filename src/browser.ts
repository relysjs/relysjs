...
import { app_ctx__be_config, middleware_ctx__be_config } from './ctx.js'
import { server__input_path_ } from './server.js'
export const [
	browser__metafile$_,
	browser__metafile_,
	browser__metafile__set
] = be_sig_triple_<Metafile|undefined>(()=>
		undefined,
			...
			(browser__metafile, server__input_path)=>{
				const browser__input_path =
					server__input_path.replace(
						...
						'.browser.$1')
				return browser__metafile.inputs[browser__input_path] && browser__input_path as string
			}),
